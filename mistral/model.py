...
def repeat_kv(keys: torch.Tensor, values: torch.Tensor, repeats: int, dim: int):
    ...
    values = torch.repeat_interleave(values, repeats, dim=dim)
    return keys, values
    ...
    def __init__(self, args: ModelArgs):
        ...
        if args.moe is not None:
            self.feed_forward = MoeLayer(
                ...
                # TODO: What if we use a BitLinear for the gate?
...
class Transformer(nn.Module):
    ...
    def forward_partial(
        ...
        assert sum(seqlens) == num_toks, (seqlens, num_toks)
    ...
    def forward(
        ...
        if self.pipeline_rank < self.num_pipeline_ranks - 1:
            ...
            # the last stage
            outs = torch.empty(
                h.shape[0], self.vocab_size, device=self.device, dtype=self.dtype
            )
        else:
        ...
        return outs.float()  # TODO: verify that BitNet doesn't use low precision here
